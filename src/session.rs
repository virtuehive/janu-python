...
use janu::prelude::{ExprId, KeyExpr as ZKeyExpr, ZFuture, ZInt};

/// A janu-net session.
#[pyclass]
pub struct Session {
    s: Option<janu::Session>,
}

#[pymethods]
impl Session {
    /// Close the janu-net Session.
    pub fn close(&mut self) -> PyResult<()> {
        let s = self.take()?;
        s.close().wait().map_err(to_pyerr)
    }

    /// Get informations about the janu-net Session.
    ///
    /// :rtype: dict {str: str}
    ///
    /// :Example:
    ...
    pub fn info(&self, py: Python) -> PyResult<PyObject> {
        use janu_util::properties::KeyTranscoder;
        let s = self.as_ref()?;
        let props = s.info().wait();
        let pydict: HashMap<String, String> = props
            .0
            .into_iter()
            .filter_map(|(k, v)| janu::info::InfoTranscoder::decode(k).map(|k| (k, v)))
            .collect();
    ...
    fn subscribe(
        ...
        let mut static_zn_sub = unsafe {
            std::mem::transmute::<
                janu::subscriber::Subscriber<'_>,
                janu::subscriber::Subscriber<'static>,
            >(zn_sub)
        };
    ...
    ///
    /// >>> import janu, time
    /// >>> from janu import Sample, queryable
    /// >>> def callback(query):
    ...
    fn queryable(&self, key_expr: &PyAny, kind: ZInt, callback: &PyAny) -> PyResult<Queryable> {
        ...
        let mut zn_quer = unsafe {
            std::mem::transmute::<
                janu::queryable::Queryable<'_>,
                janu::queryable::Queryable<'static>,
            >(zn_quer)
        };
        ...
        let mut zn_recv = match selector.get_type().name()? {
            ...
            x => {
                return Err(PyErr::new::<exceptions::PyValueError, _>(format!(
                    "Cannot convert type '{}' to a janu Selector",
                    x
                )))
            }
        };
...
impl Session {
    pub(crate) fn new(s: janu::Session) -> Self {
        Session { s: Some(s) }
    }

    #[inline]
    fn as_ref(&self) -> PyResult<&janu::Session> {
        self.s
            .as_ref()
            .ok_or_else(|| PyErr::new::<ZError, _>("janu-net session was closed"))
    }

    #[inline]
    fn take(&mut self) -> PyResult<janu::Session> {
        self.s
            .take()
            .ok_or_else(|| PyErr::new::<ZError, _>("janu-net session was closed"))
    }
