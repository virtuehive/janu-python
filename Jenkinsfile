pipeline {
  ...
  environment {
      LABEL = get_label()
      DOWNLOAD_DIR="/home/data/httpd/download.eclipse.org/janu/janu-python/${LABEL}"
  }
  ...
  stages {
    stage('Checkout Git TAG') {
      steps {
        ...
        checkout([$class: 'GitSCM',
                  ...
                  gitTool: 'Default',
                  submoduleCfg: [],
                  userRemoteConfigs: [[url: 'https://github.com/virtuehive/janu-python.git']]
                ])
      }
    }
    ...
    stage('Manylinux2010-x64 wheels') {
      ...
      steps {
        sh '''
        docker run --init --rm -v $(pwd):/workdir -w /workdir adlinktech/janu-dev-manylinux2010-x86_64-gnu maturin build --release --manylinux 2010
        '''
      }
    }
    ...
    stage('Manylinux2010-i686 wheels') {
      ...
      steps {
        sh '''
        docker run --init --rm -v $(pwd):/workdir -w /workdir adlinktech/janu-dev-manylinux2010-i686-gnu maturin build --release --manylinux 2010
        '''
      }
    }
    ...
    stage('Manylinux2014-aarch64 wheels') {
      ...
      steps {
        sh '''
        docker run --init --rm -v $(pwd):/workdir -w /workdir adlinktech/janu-dev-manylinux2014-aarch64-gnu maturin build --release --manylinux 2014
        '''
      }
    }
      ...
      steps {
        sshagent ( ['projects-storage.eclipse.org-bot-ssh']) {
          sh '''
            ...
            scp target/wheels/*.tar.gz genie.janu@projects-storage.eclipse.org:${DOWNLOAD_DIR}/
            find target -name "*.whl" | xargs -J FILES scp FILES genie.janu@projects-storage.eclipse.org:${DOWNLOAD_DIR}/
          '''
        }
      }
    }
    ...
    stage('Deploy on pypi.org') {
      ...
      steps {
        sh '''
          python3 -m twine upload --repository virtuehive `find target -name "*.whl"` target/wheels/*.tar.gz
        '''
      }
    }
  }
